...
class HandIKSolver:
    ...
    def _get_angle_limits(self, angle_name):
        ...
        if 'thumb' in angle_name:
            if 'cmc_flexion' in angle_name: return (-25, 50)
            if 'mcp_flexion' in angle_name: return (0, 80)
            if 'ip_flexion' in angle_name: return (0, 90)
            if 'abduction' in angle_name: return (-45, 45)
    ...
    def fast_calculate_joint_angles(self, landmarks):
        ...
        try:
            ...
            # 计算拇指角度
            angles['thumb_cmc_flexion'] = self._compute_flexion_angle(wrist_to_cmc, cmc_to_mcp, rotation_matrix)
            angles['thumb_cmc_abduction'] = self._compute_abduction_angle(wrist_to_cmc, cmc_to_mcp, rotation_matrix[:, 2])
            angles['thumb_mcp_flexion'] = self._compute_flexion_angle(cmc_to_mcp, mcp_to_ip, rotation_matrix)
            angles['thumb_mcp_abduction'] = self._compute_abduction_angle(cmc_to_mcp, mcp_to_ip, rotation_matrix[:, 2])
            angles['thumb_ip_flexion'] = self._compute_flexion_angle(mcp_to_ip, ip_to_tip, rotation_matrix)
    ...
    def _compute_flexion_angle(self, parent_vec, child_vec, rotation_matrix):
        """直接计算屈曲角度，无需IK"""
        # 确保向量单位化
...
if __name__ == "__main__":
    ...
    # 读取并插值数据
    file_path = "E:\multimodel-acquisition\hand_joint_data\hand_joint_angles_20250409_120031.h5"
