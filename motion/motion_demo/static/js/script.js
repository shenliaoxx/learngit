class MotionDemo {
    ...
    loadMotion(motion) {
        ...
        // 更新视频
        const videoPath = `/videos/${motion.video}`;
        this.motionVideo.src = videoPath;
        this.motionVideo.load();
        
        // 视频加载错误处理
        this.motionVideo.onerror = () => {
            console.error('视频加载失败:', videoPath);
            this.updateStatus(`视频加载失败: ${motion.name}`, true);
            this.showVideoError(`无法加载视频: ${motion.name}`);
        };
    ...
    showVideoError(message) {
        if (this.videoError) {
            this.videoError.querySelector('span').textContent = message || '视频加载失败';
